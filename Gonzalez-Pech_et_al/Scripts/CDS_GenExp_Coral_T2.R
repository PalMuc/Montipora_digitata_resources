...
countdata<-read.table("./coral_cds_stranded.counts",
...
col_deg[1:10,]
write.table(col_deg,file="./coral_col_T2_deg.txt",sep="\t")
...
cond_deg[1:10,]
write.table(cond_deg,file="./coral_cond_T2_deg.txt",sep="\t")
write.table(cond_genes,file="./coral_cond_T2_genes.txt",sep="\t")
...
write.table(cond_GOs,
            file = "./Condition_T2_BP_deg1.tab",
            quote = F, sep = "\t", row.names = F, col.names = F)
library(topGO)
cond_GOs <- readMappings(file = "./Condition_T2_BP_deg1.tab")
str(head(cond_GOs))
